...
<html lang="en">
    <head>
        ...
        <meta name="description" content="{{env('APP_NAME') ?? 'DS BOJANA'}} is an application that allows you to create menus using lists of recipes from the community">
        <link href="{{ asset('css/app.css') }}" rel="stylesheet">
        <link href="{{ asset('css/icons.css') }}" rel="stylesheet" >
        @livewireStyles
        <script src="https://kit.fontawesome.com/a357069ed8.js" crossorigin="anonymous"></script>
        <script src="{{ asset('js/app.js') }}" defer></script>
        <title>@if (trim($__env->yieldContent('title'))) @yield('title') | @endif {{env('APP_NAME') ?? 'DS BOJANA'}}</title>
        @livewireScripts
    ...
    <body>
        <div class="w-full min-h-screen flex flex-col lg:flex-row bg-gray-100 dark:bg-gray-800">
            ...
            <main class="p-4 lg:p-6 w-full lg:ml-72 relative" @auth @if(Auth::user()->hasRole('Super Admin')) style="background: url('{{secure_asset('assets/images/app.jpg')}}');background-size: cover;background-repeat: no-repeat;background-position: center;" @endif @endAuth >
                <!-- Search Bar -->
